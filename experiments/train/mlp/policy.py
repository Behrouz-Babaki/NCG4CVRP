...
class Policy(nn.Module):
    def __init__(self, learn_quadratic=True, ks=None, 
        ...
        mlp_out_dim = A_dim+1 if learn_quadratic else 1 
        
        self.enc = nn.Sequential(
            nn.Linear(in_features=5, out_features=mlp_emb_size),
            nn.LayerNorm(mlp_emb_size),
            nn.ReLU(),
            nn.Linear(in_features=mlp_emb_size, out_features=mlp_out_dim)
        )
    ...
    def forward(self, inputs):
        ...
        customer_embeddings = self.enc(customer_features)
