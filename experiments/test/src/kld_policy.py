...
class Policy(nn.Module):
    def __init__(self, learn_quadratic=True,
        ...
        self.se_module = SetEncoder(dim_input=gcn_out_dim+4, dim_output=1)
    ...
    def forward(self, inputs):
        ...
        predicted_weightss = torch.unbind(customer_embeddings.squeeze())
        adjusted_dualss = torch.split(adjusted_dualss, num_customerss.tolist(), dim=0)
        ...
        for i in range(batch_size):
            ...
            log_probs = F.log_softmax(predicted_weightss[i], dim=0)
            target_probs = adjusted_dualss[i] / dual_sums[i]
            loss = F.kl_div(log_probs, target_probs, log_target=False, reduction='batchmean')
            batch_loss += loss
            
            with torch.no_grad():
                w = F.softmax(predicted_weightss[i], dim=0) * dual_sums[i]
